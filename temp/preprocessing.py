...
train_questions_path = os.path.join(data_path, 'v2_OpenEnded_mscoco_train2014_questions.json')
val_questions_path = os.path.join(data_path, 'v2_OpenEnded_mscoco_val2014_questions.json')
train_annot_path = os.path.join(data_path, 'v2_mscoco_train2014_annotations.json')
val_annot_path = os.path.join(data_path, 'v2_mscoco_val2014_annotations.json')

train_questions = json.load(open(train_questions_path, 'r'))['questions']
val_questions = json.load(open(val_questions_path, 'r'))['questions']
train_annot = json.load(open(train_annot_path, 'r'))['annotations']
val_annot = json.load(open(val_annot_path, 'r'))['annotations']
...
def preeprocess_question(question):
    question = question.lower()
    question = ' '.join(re.findall(r"[\w']+|[.,!?;]", question))
    question = process_digit_article(question)
...
def question2indexes(question: list, q2label):
    question_indexes = []
    for q in question:
        if q in q2label:
            question_indexes.append(q2label[q])
        else:
            question_indexes.append(q2label['UNK'])
    return question_indexes


def compute_target(answers_dset, question_dset, ans2label, q2label, name, cache_root):
    ...
    ***answers_dset should be preprocessed***
    ...
    """
    target = []
    for ans_entry, q_entry in zip(answers_dset, question_dset):
        question = q_entry['question']
        question = preeprocess_question(question)
        question_idx = question2indexes(question, q2label)
...
def create_q2label(train_questions, name, cache_root):
    q2label = {'PAD': 0, 'UNK': 1}
    label2q = ['PAD', 'UNK']
    index = 2
    for question in train_questions:
        ...
        for q in question:
            if q not in q2label:
                ...
                label2q.append(q)
                index += 1
    cache_file = os.path.join(cache_root, name + '_q2label.pkl')
    pickle.dump(q2label, open(cache_file, 'wb'))
    cache_file = os.path.join(cache_root, name + '_label2q.pkl')
